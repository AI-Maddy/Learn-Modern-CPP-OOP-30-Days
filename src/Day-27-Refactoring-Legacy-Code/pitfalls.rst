...
**Description**
  ...
  verify that observable behaviour is preserved. The refactor introduces a
  regression that slips into production.
...
**Why it fails**
  The sign flip is syntactically valid. No compiler warning is emitted.
  Without a test asserting the expected values, the bug ships silently.
...
**Detection tip**
  Make it a rule: zero refactoring PRs without accompanying tests. Use CI
  to block merges unless the test suite passes.
...
**Description**
  ...
  months. The old codebase keeps evolving, making merging a nightmare and
  often leading to behaviour regressions.
...
**Why it fails**
  Both branches diverge for weeks. The merge introduces hundreds of
  conflicts. The rewrite misses edge-cases added to main during the
  rewrite period. Testing the merge is as expensive as the original work.
...
**Detection tip**
  If a refactoring PR has more than ~400 lines changed, split it.
...
**Description**
  When extracting a helper function, forgetting to mark it ``const`` (or
  removing ``const`` from a parameter type to make compilation easier)
  silently weakens the API contract and propagates mutability.
...
**Why it fails**
  The omission propagates: either the caller must remove ``const`` too
  (spreading const-incorrectness), or they get a compile error and are
  confused about why.
...
**Detection tip**
  Enable ``-Wconversion`` and ``-Wshadow``; also run clang-tidy's
  ``readability-const-return-type`` and ``cppcoreguidelines-avoid-const-
  or-ref-data-members`` checks on refactored code.
...
**Description**
  ...
  boundary condition (``<`` vs ``<=``, ``>`` vs ``>=``) while editing
...
**Why it fails**
  A boundary change at day 30 affects thousands of accounts. The developer
  did not intend it; it was a mechanical error during a rename. Without a
  test for exactly ``days_overdue == 30``, it is invisible.
...
**Detection tip**
  Include off-by-one tests for every boundary condition in the existing
  function before touching the code. Commit tests in a separate commit
  before the refactoring commit so history is clear.
...
**Description**
  ...
  refactoring a module changes compile-time dependencies. This can
  cause circular includes, increased compile times, or platform portability
  issues that were not in scope.
...
**Why it fails**
  A refactoring PR should preserve behaviour AND dependencies. Adding a
  logging library is a new feature/concern that should be a separate PR.
  It complicates review, can break builds without spdlog installed, and
  makes rolling back harder.
...
**Detection tip**
  Diff your includes: ``git diff --stat`` should show zero new ``#include``
  lines in a pure structural refactoring PR. If any appear, move them to
  a follow-up PR.
...
**Description**
  ...
  refactored legacy code means most of the smells it can catch are never
  reported.
...
**Why it fails**
  Legacy smells accumulate indefinitely. New developers copy the old style,
  thinking it is the project convention.
...
**Detection tip**
  Add a CI step that runs ``clang-tidy`` on all files changed in a PR and
  fails the build on new warnings. Use ``//NOLINT`` sparingly and always
