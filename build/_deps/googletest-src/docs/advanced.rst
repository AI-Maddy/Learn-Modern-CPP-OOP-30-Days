...
Now that you have read the `GoogleTest Primer <primer.rst>`_ and learned how to
...
See `Explicit Success and Failure <reference/assertions.md#success-failure>`_ in
...
See `Exception Assertions <reference/assertions.md#exceptions>`_ in the Assertions
Reference.
...
```EXPECT_PRED*`` <reference/assertions.md#EXPECT_PRED>`_ in the Assertions
...
```EXPECT_PRED*`` <reference/assertions.md#EXPECT_PRED>`_ and
```EXPECT_TRUE`` <reference/assertions.md#EXPECT_TRUE>`_ unsatisfactory, or some
...
```EXPECT_PRED_FORMAT*`` <reference/assertions.md#EXPECT_PRED_FORMAT>`_ in the
...
See `Floating-Point Comparison <reference/assertions.md#floating-point>`_ in the
...
```EXPECT_PRED_FORMAT2`` <reference/assertions.md#EXPECT_PRED_FORMAT>`_, for
example:
...
(Please read the `previous <#asserting-using-gmock-matchers>`_ section first if
you haven't.)

You can use the gMock `string matchers <reference/matchers.md#string-matchers>`_
with ```EXPECT_THAT`` <reference/assertions.md#EXPECT_THAT>`_ to do more string
...
See `Windows HRESULT Assertions <reference/assertions.md#HRESULT>`_ in the
Assertions Reference.
...
`constructor/destructor vs. ``SetUp``/``TearDown`` <faq.md#CtorVsSetUp>`_
...
code, see `Exception Assertions <#ExceptionAssertions>`_.

If you want to test ``EXPECT_*()/ASSERT_*()`` failures in your test code, see
`"Catching" Failures <#catching-failures>`_.
...
`Death Assertions <reference/assertions.md#death>`_ in the Assertions Reference
for details.
...
`Death Tests And Threads <#death-tests-and-threads>`_ section below explains why.
...
`RE2 <https://github.com/google/re2/wiki/Syntax>`_ syntax. Otherwise, for POSIX
systems (Linux, Cygwin, Mac), GoogleTest uses the
`POSIX extended regular expression <http://www.opengroup.org/onlinepubs/009695399/basedefs/xbd_chap09.html#tag_09_04>`_
syntax. To learn about POSIX syntax, you may want to read this
`Wikipedia entry <http://en.wikipedia.org/wiki/Regular_expression#POSIX_extended>`_.
...
See `Death Assertions <reference/assertions.md#death>`_ in the Assertions
Reference.
...
#. A warning is emitted if multiple threads are running when a death test is
    encountered.
#. Test suites with a name ending in "DeathTest" are run before all other
    tests.
...
`a custom GMock matcher <gmock_cook_book.md#NewMatchers>`_ instead. This lets you
...
`XML output <#generating-an-xml-report>`_ if you specify one. For example, the
test
...
#. In the same test fixture class, define a ``static void SetUpTestSuite()``
...
```INSTANTIATE_TEST_SUITE_P`` <reference/testing.md#INSTANTIATE_TEST_SUITE_P>`_ in
...
```Values`` <reference/testing.md#param-generators>`_ parameter generator:
...
`parameter generator <reference/testing.md#param-generators>`_.
...
You can use these names in ```--gtest_filter`` <#running-a-subset-of-the-tests>`_.

The following statement will instantiate all tests from ``FooTest`` again, each
with parameter values ``"cat"`` and ``"dog"`` using the
```ValuesIn`` <reference/testing.md#param-generators>`_ parameter generator:
...
`should not contain underscores <faq.md#why-should-test-suite-names-and-test-names-not-contain-underscore>`_
...
*   Private class members are only accessible from within the class or by
    ...
    called the
    `Pimpl <https://www.gamedev.net/articles/programming/general-and-gameplay-programming/the-c-pimpl-r1794/>`_
...
```TestInfo`` <reference/testing.md#TestInfo>`_ class has this information.

To obtain a ``TestInfo`` object for the currently running test, call
``current_test_info()`` on the ```UnitTest`` <reference/testing.md#UnitTest>`_
singleton object:
...
```testing::TestEventListener`` <reference/testing.md#TestEventListener>`_ or
```testing::EmptyTestEventListener`` <reference/testing.md#EmptyTestEventListener>`_
...
```TestEventListeners`` <reference/testing.md#TestEventListeners>`_ - note the "s"
...
`global set-up/tear-down <#global-set-up-and-tear-down>`_ code, it will be
...
#. On each shard, set the ``GTEST_TOTAL_SHARDS`` environment variable to the total
    number of shards. It must be the same for all shards.
#. On each shard, set the ``GTEST_SHARD_INDEX`` environment variable to the index
...
`JSON encoding <https://developers.google.com/protocol-buffers/docs/proto3#json>`_:
...
The
`Undefined Behavior Sanitizer <https://clang.llvm.org/docs/UndefinedBehaviorSanitizer.html>`_,
`Address Sanitizer <https://github.com/google/sanitizers/wiki/AddressSanitizer>`_,
and
`Thread Sanitizer <https://github.com/google/sanitizers/wiki/ThreadSanitizerCppManual>`_
