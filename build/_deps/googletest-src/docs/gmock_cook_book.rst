...
`the dummy guide <gmock_for_dummies.rst>`_ first to make sure you understand the
basics.
...
recipe for `mocking non-virtual methods <#MockingNonVirtualMethods>`_.
...
`MockFunction <#MockFunction>`_ to mock the std::function.
...
`was introduced in 2018 <https://github.com/google/googletest/commit/c5f08bf91944ce1b19bcf414fa1760e69d20afc2>`_,
...
`Understanding Uninteresting vs Unexpected Calls <#uninteresting-vs-unexpected>`_.

There are some caveats though (sadly they are side effects of C++'s
limitations):

#. ``NiceMock<MockFoo>`` and ``StrictMock<MockFoo>`` only work for mock methods
    defined using the ``MOCK_METHOD`` macro **directly** in the ``MockFoo`` class.
    If a mock method is defined in a **base class** of ``MockFoo``, the "nice" or
    "strict" modifier may not affect it, depending on the compiler. In
    particular, nesting ``NiceMock`` and ``StrictMock`` (e.g.
    ``NiceMock<StrictMock<MockFoo> >``) is **not** supported.
#. ``NiceMock<MockFoo>`` and ``StrictMock<MockFoo>`` may not work correctly if the
...
*   The general technique discussed here works for overloaded methods, but
    ...
    use `this technique <#SelectOverload>`_; to disambiguate a fake function (the
...
This technique is very similar to the `delegating-to-fake <#DelegatingToFake>`_
...
#. When both ``T`` and ``U`` are built-in arithmetic types (``bool``, integers, and
    floating-point numbers), the conversion from ``T`` to ``U`` is not lossy (in
    other words, any value representable by ``T`` can also be represented by ``U``);
    and
#. When ``U`` is a reference, ``T`` must also be a reference (as the underlying
...
`Multi-argument Matchers <reference/matchers.md#MultiArgMatchers>`_ for the
complete list.
...
See ```EXPECT_THAT`` <reference/assertions.md#EXPECT_THAT>`_ in the Assertions
Reference.
...
values—see the `Matchers Reference <reference/matchers.rst>`_ for more information.
...
that there are ```AllOf()`` and ``AllOfArray()`` <#CombiningMatchers>`_.
...
Most matchers can be simply defined using `the MATCHER* macros <#NewMatchers>`_,
...
``DefaultValue<T>::Set()`` (described `here <#DefaultValue>`_) or ``ON_CALL()``.
...
`"The Nice, the Strict, and the Naggy" <#NiceStrictNaggy>`_.
...
supplying a `simpler mock interface <#SimplerInterfaces>`_ than the mocked class
provides.

This pattern is also useful when the arguments are interesting, but match logic
is substantially complex. You can leave the argument list unspecified and use
SaveArg actions to `save the values for later verification <#SaveArgVerify>`_. If
...
```After`` clause <reference/mocking.md#EXPECT_CALL.After>`_ of ``EXPECT_CALL``.
...
The `selecting-an-action's-arguments <#SelectingArgs>`_ recipe showed us one way
...
`workarounds <#LegacyMoveOnly>`_ for lack of this feature.
...
`Setting Default Actions <#OnCall>`_.
...
can always use ``Return``, or a `lambda or functor <#FunctionsAsActions>`_:
...
that you can specify constraints on the `order <#OrderedCalls>`_ of mock function
...
#. Set its default actions and expectations using ``ON_CALL()`` and
    ``EXPECT_CALL()``.
#. The code under test calls methods of ``foo``.
#. Optionally, verify and reset the mock.
#. Destroy the mock yourself, or let the code under test destroy it. The
...
`this section <#PureMatchers>`_ for more details.
...
If the `built-in set <gmock_cheat_sheet.md#CardinalityList>`_ of cardinalities
...
`The GoogleTest advanced guide <advanced.md#teaching-googletest-how-to-print-your-values>`_
