...
#. Tests should be *portable* and *reusable*. Google has a lot of code that is
...
#. Tests should be *fast*. With GoogleTest, you can reuse shared resources
...
Testing Qualifications Board (`ISTQB <http://www.istqb.org/>`_) materials and
...
Meaning                                                                              | GoogleTest Term         | `ISTQB <http://www.istqb.org/>`_ Term
:----------------------------------------------------------------------------------- | :---------------------- | :----------------------------------
Exercise a particular program path with specific input values and verify the results | `TEST() <#simple-tests>`_ | [Test Case][istqb test case]
...
the ```ASSERT_EQ`` and ``EXPECT_EQ`` <reference/assertions.md#EXPECT_EQ>`_ macros to
...
provided by GoogleTest, see the `Assertions Reference <reference/assertions.rst>`_.
...
for
`naming functions and classes <https://google.github.io/styleguide/cppguide.html#Function_Names>`_.
...
> `death tests <advanced.md#death-tests>`_) and thus is not supported.
...
`AdvancedGuide <advanced.rst>`_. You **must** call this function before calling
