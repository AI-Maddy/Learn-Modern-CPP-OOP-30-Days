...
`Assertions <assertions.rst>`_ used within the test body determine the outcome of
the test.
...
`Test Fixtures <../primer.md#same-data-multiple-tests>`_.
...
`Value-Parameterized Tests <../advanced.md#value-parameterized-tests>`_.
...
```WithParamInterface`` <#WithParamInterface>`_). For example:
...
See also ```INSTANTIATE_TEST_SUITE_P`` <#INSTANTIATE_TEST_SUITE_P>`_.
...
```TestParamInfo<class ParamType>`` <#TestParamInfo>`_ and return a ``std::string``.
The test name suffix can only contain alphanumeric characters and underscores.
GoogleTest provides ```PrintToStringParamName`` <#PrintToStringParamName>`_, or a
...
See also
```GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST`` <#GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST>`_.
...
The argument *``Types``* is a ```Types`` <#Types>`_ object representing the list of
...
See also ```TYPED_TEST`` <#TYPED_TEST>`_ and
`Typed Tests <../advanced.md#typed-tests>`_ for more information.
...
```TYPED_TEST_SUITE`` <#TYPED_TEST_SUITE>`_.
...
For more information, see `Typed Tests <../advanced.md#typed-tests>`_.
...
The argument *``TestFixtureName``* is a fixture class template, parameterized by a
type. See ```TYPED_TEST_SUITE`` <#TYPED_TEST_SUITE>`_ for an example.

See also ```TYPED_TEST_P`` <#TYPED_TEST_P>`_ and
`Type-Parameterized Tests <../advanced.md#type-parameterized-tests>`_ for more
information.
...
with ```TYPED_TEST_SUITE_P`` <#TYPED_TEST_SUITE_P>`_.

Within the test body, the special name ``TypeParam`` refers to the type parameter,
and ``TestFixture`` refers to the fixture class. See ```TYPED_TEST`` <#TYPED_TEST>`_
for an example.

See also ```REGISTER_TYPED_TEST_SUITE_P`` <#REGISTER_TYPED_TEST_SUITE_P>`_ and
`Type-Parameterized Tests <../advanced.md#type-parameterized-tests>`_ for more
information.
...
```TYPED_TEST_SUITE_P`` <#TYPED_TEST_SUITE_P>`_ and ```TYPED_TEST_P`` <#TYPED_TEST_P>`_.
...
See also ```INSTANTIATE_TYPED_TEST_SUITE_P`` <#INSTANTIATE_TYPED_TEST_SUITE_P>`_ and
`Type-Parameterized Tests <../advanced.md#type-parameterized-tests>`_ for more
information.
...
```REGISTER_TYPED_TEST_SUITE_P`` <#REGISTER_TYPED_TEST_SUITE_P>`_.
...
For more information, see
`Type-Parameterized Tests <../advanced.md#type-parameterized-tests>`_.
...
See `Testing Private Code <../advanced.md#testing-private-code>`_ for more
information.
...
For more information, see
`Adding Traces to Assertions <../advanced.md#adding-traces-to-assertions>`_.

See also the ```ScopedTrace`` class <#ScopedTrace>`_.
...
```Environment`` <#Environment>`_ or ```Test`` <#Test>`_ classes). If used in a global
...
See `Skipping Test Execution <../advanced.md#skipping-test-execution>`_ for more
information.
...
By default, every ```TEST_P`` <#TEST_P>`_ call without a corresponding
```INSTANTIATE_TEST_SUITE_P`` <#INSTANTIATE_TEST_SUITE_P>`_ call causes a failing
...
```AssertionSuccess()`` <#AssertionSuccess>`_ or
```AssertionFailure()`` <#AssertionFailure>`_.
...
```TestEventListener::OnTestPartResult`` <#TestEventListener::OnTestPartResult>`_.
...
```TestEventListener`` <#TestEventListener>`_ interface, such that a subclass only
...
`Global Set-Up and Tear-Down <../advanced.md#global-set-up-and-tear-down>`_.
...
See also ```SCOPED_TRACE`` <#SCOPED_TRACE>`_.
...
A convenience class which inherits from both ```Test`` <#Test>`_ and
```WithParamInterface<T>`` <#WithParamInterface>`_.
...
type-parameterized test suite. See `Typed Tests <../advanced.md#typed-tests>`_ and
`Type-Parameterized Tests <../advanced.md#type-parameterized-tests>`_.
...
Returns the ```TestInfo`` <#TestInfo>`_ for the ``i``-th test among all the tests. ``i``
...
Returns the ```TestResult`` <#TestResult>`_ that holds test properties recorded
...
Returns the name of the parameter type, or ``NULL`` if this is not a typed or
type-parameterized test. See `Typed Tests <../advanced.md#typed-tests>`_ and
`Type-Parameterized Tests <../advanced.md#type-parameterized-tests>`_.
...
`Running a Subset of the Tests <../advanced.md#running-a-subset-of-the-tests>`_
...
Returns the result of the test. See ```TestResult`` <#TestResult>`_.
...
Returns the working directory when the first ```TEST()`` <#TEST>`_ or
```TEST_F()`` <#TEST_F>`_ was executed. The ``UnitTest`` object owns the string.
...
Returns the ```TestSuite`` <#TestSuite>`_ object for the test that's currently
...
Returns the ```TestInfo`` <#TestInfo>`_ object for the test that's currently
running, or ``NULL`` if no test is running.
...
Gets the ```TestSuite`` <#TestSuite>`_ object for the ``i``-th test suite among all
...
Returns the ```TestResult`` <#TestResult>`_ containing information on test failures
...
GoogleTest. See ```TestEventListeners`` <#TestEventListeners>`_.
...
```AssertionException`` <#AssertionException>`_ or inherited from it.
...
```Release()`` <#TestEventListeners::Release>`_ transfers its ownership to the
...
list with ```Release()`` <#TestEventListeners::Release>`_ transfers its ownership to
...
Returns the ```TestPartResult`` <#TestPartResult>`_ for the ``i``-th test part result
...
Returns the ```TestProperty`` <#TestProperty>`_ object for the ``i``-th test property.
...
See `Typed Tests <../advanced.md#typed-tests>`_ and
`Type-Parameterized Tests <../advanced.md#type-parameterized-tests>`_ for more
information.
...
A value-parameterized test fixture class must inherit from both ```Test`` <#Test>`_
and ``WithParamInterface``. In most cases that just means inheriting from
```TestWithParam`` <#TestWithParam>`_, but more complicated test hierarchies may
...
```RUN_ALL_TESTS()`` <#RUN_ALL_TESTS>`_. In particular, it parses the command line
...
```RUN_ALL_TESTS()`` <#RUN_ALL_TESTS>`_ is called. See
`Global Set-Up and Tear-Down <../advanced.md#global-set-up-and-tear-down>`_ for
more information.

See also ```Environment`` <#Environment>`_.
...
Must be called before ```RUN_ALL_TESTS()`` <#RUN_ALL_TESTS>`_ is invoked, otherwise
behavior is undefined.

See
`Registering tests programmatically <../advanced.md#registering-tests-programmatically>`_
for more information.
...
```InitGoogleTest()`` <#InitGoogleTest>`_.
...
```AssertionResult`` <#AssertionResult>`_.
...
See ```AssertionResult`` <#AssertionResult>`_.
...
See `Type Assertions <../advanced.md#type-assertions>`_ for more information.
...
```PrintToString`` <#PrintToString>`_ called on ``info.param``. Does not work when the
test parameter is a ``std::string`` or C string. See
`Specifying Names for Value-Parameterized Test Parameters <../advanced.md#specifying-names-for-value-parameterized-test-parameters>`_
for more information.

See also ```TestParamInfo`` <#TestParamInfo>`_ and
```INSTANTIATE_TEST_SUITE_P`` <#INSTANTIATE_TEST_SUITE_P>`_.
