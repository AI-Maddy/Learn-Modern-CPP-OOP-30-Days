...
Creates an `expectation <../gmock_for_dummies.md#setting-expectations>`_ that the
...
`matchers <../gmock_for_dummies.md#matchers-what-arguments-do-we-expect>`_ that
...
each argument's matcher were a `wildcard matcher (``_``) <matchers.md#wildcard>`_.
See the `Matchers Reference <matchers.rst>`_ for a list of all built-in matchers.
...
matcher above. See `Multi-argument Matchers <matchers.md#MultiArgMatchers>`_.
...
*   If neither ```WillOnce`` <#EXPECT_CALL.WillOnce>`_ nor
    ```WillRepeatedly`` <#EXPECT_CALL.WillRepeatedly>`_ are specified, the inferred
...
The parameter *``sequences...``* is any number of ```Sequence`` <#Sequence>`_ objects.
...
See also the ```InSequence`` class <#InSequence>`_.
...
```Expectation`` <#Expectation>`_ or ```ExpectationSet`` <#ExpectationSet>`_ objects.
...
`action <../gmock_for_dummies.md#actions-what-should-it-do>`_ that the function
call will perform. See the `Actions Reference <actions.rst>`_ for a list of
built-in actions.

The use of ``WillOnce`` implicitly sets a cardinality on the expectation when
``Times`` is not specified. See ```Times`` <#EXPECT_CALL.Times>`_.
...
```WillOnce`` <#EXPECT_CALL.WillOnce>`_ clauses, if any, have been performed.
...
when ``Times`` is not specified. See ```Times`` <#EXPECT_CALL.Times>`_.
...
```WillOnce`` <#EXPECT_CALL.WillOnce>`_ and
```WillRepeatedly`` <#EXPECT_CALL.WillRepeatedly>`_ clauses of ``EXPECT_CALL``.
...
`uninteresting calls <../gmock_cook_book.md#uninteresting-vs-unexpected>`_. The
...
```InSequence`` <#EXPECT_CALL.InSequence>`_ clause of ``EXPECT_CALL`` for usage.
...
```After`` <#EXPECT_CALL.After>`_ clause of ``EXPECT_CALL``.
...
Use the ``+=`` operator to add ```Expectation`` <#Expectation>`_ objects to the set:
