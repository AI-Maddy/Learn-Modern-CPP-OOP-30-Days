...
`Assertion Placement <../advanced.md#assertion-placement>`_ for more information.
...
The following assertion allows `matchers <matchers.rst>`_ to be used to verify
values.
...
Verifies that *``value``* matches the `matcher <matchers.rst>`_ *``matcher``*.
...
`Matchers Reference <matchers.rst>`_. It is also possible to write your own
matchers—see `Writing New Matchers Quickly <../gmock_cook_book.md#NewMatchers>`_.
...
`Teaching GoogleTest How to Print Your Values <../advanced.md#teaching-googletest-how-to-print-your-values>`_.
...
See also the `Floating-Point Comparison <#floating-point>`_ assertions to compare
...
```EXPECT_STREQ`` <#EXPECT_STREQ>`_ to compare C strings (e.g. ``const char*``) by
value.
...
```EXPECT_STRNE`` <#EXPECT_STRNE>`_ to compare C strings (e.g. ``const char*``) by
value.
...
objects, use ```EXPECT_EQ`` <#EXPECT_EQ>`_ or ```EXPECT_NE`` <#EXPECT_NE>`_ instead.
...
`Comparing Floating Point Numbers <https://randomascii.wordpress.com/2012/02/25/comparing-floating-point-numbers-2012-edition/>`_.
...
`Using a Function That Returns an AssertionResult <../advanced.md#using-a-function-that-returns-an-assertionresult>`_.
...
terminate. For context, see `Death Tests <../advanced.md#death-tests>`_.
...
The parameter *``matcher``* is either a `matcher <matchers.rst>`_ for a `const
std::string&`, or a regular expression (see
`Regular Expression Syntax <../advanced.md#regular-expression-syntax>`_)—a bare
string *``s``* (with no matcher) is treated as
```ContainsRegex(s)`` <matchers.md#string-matchers>`_, **not**
```Eq(s)`` <matchers.md#generic-comparison>`_.
...
```EXPECT_DEATH`` <#EXPECT_DEATH>`_. Otherwise, verifies nothing.
...
In debug mode, behaves the same as ```EXPECT_DEATH`` <#EXPECT_DEATH>`_. When not in
