...
# :material-alert: 04 — Pitfalls: Code Review Common Pitfalls
...
??? pitfall-lobe "⚠️ Pitfall 1: Approving a PR Without Reading the Logic"
    **Description**  
    ...
    **Why it fails**  
    Automated tools check formatting. Code review exists precisely to catch what automation misses: wrong algorithms, missing edge cases, and architectural problems.
    ...
    **Detection tip**  
    Add a mandatory checklist to your PR template. Reviewers must check each box before submitting approval.

??? pitfall-lobe "⚠️ Pitfall 2: Missing `const` Cascades"
    **Description**  
    ...
    **Why it fails**  
    The "fix" removes a safety guarantee. The function now must accept a mutable reference, preventing callers from passing temporary or const objects. The problem propagates upward.
    ...
    **Detection tip**  
    `clang-tidy` check `readability-make-member-function-const` flags member functions that do not modify any member and are not yet const.

??? pitfall-lobe "⚠️ Pitfall 3: Raw Owning Pointer Returned From a Factory"
    **Description**  
    ...
    **Why it fails**  
    The ownership contract is not expressed in the type. Any code path that does not reach `delete w` causes a memory leak.
    ...
    **Detection tip**  
    `clang-tidy` check `cppcoreguidelines-owning-memory` warns when a raw pointer is returned from a function that allocates with `new`.

??? pitfall-lobe "⚠️ Pitfall 4: Signed/Unsigned Comparison in Loop Bounds"
    **Description**  
    Comparing a signed loop counter to an unsigned `size()` promotes the signed value to unsigned. A negative value wraps to a huge positive, causing an out-of-bounds access or an infinite loop.
    ...
    **Why it fails**  
    The implicit conversion of `int` to `size_t` in the comparison is legal but surprising. Negative values become enormous unsigned numbers.
    ...
    **Detection tip**  
    `-Wsign-compare` (included in `-Wall`) flags signed/unsigned comparisons. Fix all such warnings — do not suppress them.

??? pitfall-lobe "⚠️ Pitfall 5: Implicit Conversion Hiding a Type Mismatch"
    **Description**  
    ...
    **Why it fails**  
    The compiler converts `3.5` to `3` without a diagnostic under the default warning settings. The discount is wrong without any indication.
    ...
    **Detection tip**  
    Enable `-Wconversion` and `-Wdouble-promotion`. Use brace-init wherever a narrowing conversion would be a bug.

??? pitfall-lobe "⚠️ Pitfall 6: `std::endl` in Performance-Critical Output"
    **Description**  
    ...
    **Why it fails**  
    For 10,000 entries, this calls `fflush` 10,000 times. The buffer would flush automatically at program exit or when full, making the per-line flush pointless and expensive.
    ...
    **Detection tip**  
    `clang-tidy` check `performance-avoid-endl` flags `std::endl` usage and suggests `'\n'`. Enable it in your `.clang-tidy` configuration.
...
Use this before pushing code from Day 28:
