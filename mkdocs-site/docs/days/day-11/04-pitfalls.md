...
# :material-alert: 04 — Pitfalls: Generic OOP Design
...
??? pitfall-lobe "⚠️ Pitfall 1: Policy Type Exposing Too Much — Breaking Encapsulation"
    ...
    **Why it fails**: Public inheritance from a policy exposes its entire interface. Callers can call policy methods directly, bypassing the host's logic.
    ...
    **Detection tip**: Policies that are implementation details should use private inheritance or containment. Public inheritance is appropriate only for IS-A relationships where the policy's public interface should be part of the host's.

??? pitfall-lobe "⚠️ Pitfall 2: Unconstrained Policy Parameters — Opaque Errors"
    ...
    **Why it fails**: Without constraints, the compiler substitutes the types and only reports errors when it reaches the point of use inside the template — far from the mistake.
    ...
    **Detection tip**: Every template policy parameter should have a concept. If you find yourself reading a 40-line instantiation chain, add concepts.

??? pitfall-lobe "⚠️ Pitfall 3: Template Method Pattern — Calling Non-Existent Hook"
    **Description**: In the template method pattern via CRTP or templates, calling a hook that the policy/derived class has not implemented compiles in some cases and silently does nothing, or calls a wrong overload.
    ...
    **Why it fails**: `post_process()` does nothing because it doesn't forward to the derived class. The omission is silent; the algorithm's contract is broken.
    ...
    **Detection tip**: Use concepts or `static_assert(requires(Derived d) { d.do_post(); })` to enforce that all hooks are implemented.

??? pitfall-lobe "⚠️ Pitfall 4: Mixing Compile-Time and Runtime Polymorphism Incorrectly"
    ...
    **Why it fails**: `Widget<BluePolicy>` and `Widget<GreenPolicy>` share no common base class and are entirely separate types. You cannot store them together.
    ...
    **Detection tip**: If you need a heterogeneous container of policy-based objects, add a thin virtual interface layer on top.

??? pitfall-lobe "⚠️ Pitfall 5: Ring Buffer — Off-By-One in Modular Indexing"
    ...
    **Why it fails**: `tail_++` is never taken modulo `N`, so it walks off the end of the array after `N` pushes.
    ...
    **Detection tip**: Always maintain a separate `count_` or `full` flag; do not rely solely on `head_ == tail_` to distinguish full from empty. Use AddressSanitizer (`-fsanitize=address`) during testing to catch out-of-bounds writes.
...
Use this before pushing code from Day 11:
