...
# :material-alert: 04 — Pitfalls: Error Handling Expected C++23
...
??? pitfall-lobe "⚠️ Pitfall 1: Ignoring the Return Value of `std::expected`"
    ...
    **Why it fails**: Unlike exceptions, `std::expected` does not automatically propagate. If you do not inspect the return value, the error is gone.
    ...
    **Detection tip**: Add `[[nodiscard]]` to every function returning `std::expected`. GCC and Clang emit a warning when the result is discarded.

??? pitfall-lobe "⚠️ Pitfall 2: Accessing `std::optional` Value Without a Check"
    **Description**: Calling `*opt` or `opt.value()` on an empty `std::optional` is either undefined behaviour (`operator*`) or an exception (`value()` throws `std::bad_optional_access`).
    ...
    **Why it fails**: The standard says dereferencing an empty optional is UB. The `value()` overload throws, but an uncaught throw terminates the program.
    ...
    **Detection tip**: Clang-tidy `bugprone-unchecked-optional-access` warns when an optional is dereferenced without a prior check.

??? pitfall-lobe "⚠️ Pitfall 3: Using `std::expected` for Logic Errors — Misuse of the Abstraction"
    **Description**: Using `std::expected` to report programming errors (null pointer dereference, out-of-bounds, precondition violations) forces callers to handle errors that should never occur in correct code.
    ...
    **Why it fails**: Logic errors cannot be "handled" by callers — they indicate a defect in the program. Surfacing them as `expected` errors suggests callers should do something when they see them, which is misleading.
    ...
    **Detection tip**: Ask: "Can a correct program ever reach this error path?" If no, it is a programming error — use assertions. If yes, use `std::expected`.

??? pitfall-lobe "⚠️ Pitfall 4: Throwing from a Destructor"
    ...
    **Why it fails**: During stack unwinding, destructors are called. If a destructor throws while unwinding from another exception, `std::terminate` is called (C++ standard §15.5.1).
    ...
    **Detection tip**: `-Wno-exceptions` and GCC's `-fno-exceptions` combined with clang-tidy `bugprone-exception-escape` will warn about exceptions escaping destructors.

??? pitfall-lobe "⚠️ Pitfall 5: Monadic Chains — Forgetting Error Type Consistency"
    **Description**: `.and_then` requires that the transformation function returns an `std::expected` with the **same error type**. Mixing error types breaks the chain.
    ...
    **Why it fails**: `and_then` on `std::expected<T, E>` requires the callable to return `std::expected<U, E>` — the same `E`. Returning a different error type breaks the type chain.
    ...
    **Detection tip**: Design the error type hierarchy upfront. A single `enum class AppError` or `std::error_code` type used throughout a module keeps `and_then` chains consistent.
...
Use this before pushing code from Day 15:
