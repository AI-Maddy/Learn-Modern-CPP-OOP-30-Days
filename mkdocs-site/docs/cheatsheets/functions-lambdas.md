...
# :material-book: Functions Lambdas
...
Function signatures, overload resolution, default arguments, attributes, and the complete lambda feature set for Modern C++ (C++14/17/20).
...
- `cheatsheets/variables-types-constexpr.rst` — auto deduction rules
- `cheatsheets/move-semantics-gotchas.rst` — perfect forwarding in template functions
- `cheatsheets/raii-smart-pointers.rst` — lambdas as custom deleters
- `cheatsheets/advanced-oop-patterns.rst` — Strategy pattern with std::function
- `cheatsheets/templates-concepts.rst` — concepts to constrain callable template params
