...
# :material-book: Structured Bindings
...
Structured bindings (C++17) unpack arrays, pairs, tuples, and aggregates into named variables in a single declaration.
...
The qualifiers on `auto` apply to the hidden underlying object, not to the individual bindings.
...
Any type can support structured bindings by specialising `std::tuple_size` and `std::tuple_element`, and providing `get<I>`.
