---
title: "Performance Tips Oop"
tags: ["cheatsheet", "reference"]
---

# :material-book: Performance Tips Oop
...
Amdahl's Law: the speedup from optimising a part of the program is bounded by the fraction of time that part occupies.
...
**Workflow**: measure → identify hotspot → optimise hotspot → re-measure. Optimising non-hotspots is wasted effort.
...
4.  Branch predictor typically **fails** on first call; subsequent calls to the same concrete type are predicted correctly
...
The compiler can eliminate the virtual dispatch when it can prove the dynamic type at compile time:
...
|--------|----------------------------------------------------|--------------------------------------------------|
| AoS    | Working on one object (all fields together)        | Bulk operations on one field across many objects |
| SoA    | Batch processing, SIMD, cache-friendly field scans | Working on one full particle at a time           |
...
Return Value Optimisation (RVO) and Named RVO eliminate the copy/move when returning a local object — the object is constructed directly in the caller's stack frame.
