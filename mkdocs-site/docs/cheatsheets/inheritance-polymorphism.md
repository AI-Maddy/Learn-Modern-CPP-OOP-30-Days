...
# :material-book: Inheritance Polymorphism
...
Access specifiers, vtable mechanics, override/final, the object slicing problem, safe casting, and Liskov Substitution for Modern C++ (C++11/17/20).
...
The access specifier on the base class controls how the outside world sees the inherited members, not whether the derived class itself can see them.
...
Each class with at least one virtual function gets a *vtable* (virtual function table). Each object carries a hidden `vptr` to its class's vtable.
...
`override` — compiler verifies function actually overrides a base virtual. `final` — prevents further overrides (on a function) or inheritance (on a class).
...
**If a class has any virtual function, its destructor must be virtual.** Without it, deleting through a base pointer skips the derived destructor — UB.
...
`dynamic_cast` performs a runtime type check. Returns null (pointer) or throws `std::bad_cast` (reference) if the cast is invalid.
...
`static_cast` does NOT check at runtime. Use only when you are certain of the dynamic type.
