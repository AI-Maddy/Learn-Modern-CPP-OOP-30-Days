---
title: "Variables Types Constexpr"
tags: ["cheatsheet", "reference"]
---

# :material-book: Variables Types Constexpr
...
Fundamental types, compile-time computation, and initialization best practices for Modern C++ (C++17/20).
...
Use `<cstdint>` types when exact width matters. Platform types (`int`, `long`) have implementation-defined sizes.
...
`auto` strips top-level cv-qualifiers and references. Know the rules to avoid surprises.
...
| Feature     | Type-safe | Scoped | Debuggab | When to use        |
|-------------|-----------|--------|----------|--------------------|
...
`std::byte` (C++17) is neither an integer nor a character — prevents accidental arithmetic on raw bytes.
...
- `docs/day-index.rst` — Day 1-3 cover types and initialization
- `cheatsheets/move-semantics-gotchas.rst` — lvalue/rvalue taxonomy
- `cheatsheets/functions-lambdas.rst` — auto return type deduction
- `cheatsheets/structured-bindings.rst` — extended binding examples
- `cheatsheets/uniform-initialization.rst` — full initialization deep-dive
