...
# :material-book: Optional Variant Any
...
Models a value that may or may not be present. Better than returning a sentinel value (`-1`, `nullptr`, `""`), because absence is explicit in the type.
...
A type-safe discriminated union. Holds exactly one of a finite set of types at a time.
...
|-------------------------|--------------------------------------------------------------------------------------|------------------------------------------------------------------------------------|
| `optional<T>`           | Value may legitimately be absent; single type; no error reason needed                | Using as error channel (use expected instead); storing bool and testing engagement |
| `variant<Ts...>`        | Finite, known set of alternative types; want exhaustiveness checking at compile time | Type set can grow; need open extensibility                                         |
| `any`                   | Open type set; configuration bags; scripting bridge; heterogeneous property maps     | Performance-critical code; type-safe access guaranteed                             |
| `expected<T,E>` (C++23) | May fail with typed error; functional chaining needed                                | Exceptions preferred; simple absence (use optional)                                |
...
| Compile-time exhaustiveness | Full     | Full    | None      | Full     |
