...
# :material-book: Ranges And Views
...
The `|` operator chains view adaptors. Each adaptor wraps the previous range lazily — no intermediate containers are allocated.
...
|------------------------------------|-------------------------------------------------|-------------------------------------------------------|
| No work until iterated             | Computes everything immediately                 | Lazy: streaming, large data, short-circuit needed     |
| No intermediate allocation         | Returns new container                           | Eager: need random access, result used multiple times |
| `views::filter` `views::transform` | `std::ranges::copy_if` `std::ranges::transform` |                                                       |
...
A **borrowed range** is one whose iterators remain valid after the range object is destroyed (e.g., `std::string_view`, `std::span`).
