---
title: "Move Semantics Gotchas"
tags: ["cheatsheet", "reference"]
---

# :material-book: Move Semantics Gotchas
...
Value categories, rvalue reference syntax, move constructor mechanics, std::move as a cast, NRVO, moved-from state, and common pitfalls.
...
The move constructor transfers resources from the source object, leaving it in a valid but unspecified state.
...
`std::move` does **nothing at runtime**. It is a static_cast to rvalue reference. The actual "move" is the constructor or assignment operator that the compiler then selects.
...
The standard requires that moved-from objects be *destructible* and *assignable*. Beyond that, their state is "valid but unspecified".
...
- `cheatsheets/rule-of-5-cheat.rst` — when move ctor/assignment are generated
- `cheatsheets/raii-smart-pointers.rst` — unique_ptr move semantics
- `cheatsheets/functions-lambdas.rst` — forwarding in function templates
- `cheatsheets/classes-constructors-raii.rst` — member initialization interaction with move
- `cheatsheets/templates-concepts.rst` — std::is_nothrow_move_constructible
