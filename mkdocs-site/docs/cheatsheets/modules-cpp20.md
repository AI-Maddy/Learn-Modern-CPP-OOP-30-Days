...
Modules replace the preprocessor-based `#include` model with a language-level mechanism that provides true encapsulation, faster builds, and eliminates macro leakage across translation units. They are the biggest change to the C++ compilation model since the language was standardised.
...
Without modules the compiler must re-parse every `#include` for every TU. With modules each interface is parsed **once** and the BMI is memory-mapped.

## Module Interface Unit

The file that `export`s declarations. Conventionally named `<module>.cppm` or `<module>-<partition>.cppm`.
...
Splits the implementation out of the interface to keep the interface file small and reduce recompilation when only the body changes.
...
No angle brackets or quotes for named modules. The compiler locates the BMI via the build system (CMake, build2, etc.).

## Header Units

A transitional feature — wrap an existing header in a module-like interface without porting it.
...
Header units are **not** the same as `import std;` (C++23). They still process macros, but the result is cached per-TU.

## `import std;` (C++23)

Imports the entire standard library as a single named module — the cleanest option when the toolchain supports it.
...
Code that must remain in the preprocessor world (e.g., macros from C headers) goes in the **global module fragment** before the `module` declaration.
...
The `FILE_SET CXX_MODULES` annotation tells CMake which files are module interfaces so it can order compilation correctly.
...
Inline functions and templates exported from a module still have definitions that must be consistent across the program. Modules prevent *accidental* duplication via headers; deliberate violations are still UB.

**Pitfall 6 — assuming all compilers handle modules identically**

BMI formats are compiler-specific (.pcm for Clang, .ifc for MSVC). Cross-compiler module sharing is not possible; each toolchain builds its own BMI.
...
- Day module: `src/Day-16-Modules-Basics-C++20/`
- Builds on: `src/Day-00-Setup-And-Basics/` (compilation pipeline)
