...
| --- | --- | --- |
| Dispatch mechanism | Indirect call via vtable pointer | Direct call (inlineable) |
| Overhead per call | ~1 indirect jmp + icache miss | Zero (often inlined away) |
| Runtime polymorphism | Yes (store Base*) | No (must know type at compile time) |
| sizeof overhead per object | +1 pointer (vptr) | None |
| Compiler optimisation | Devirtualisation sometimes possible | Full inlining possible |
