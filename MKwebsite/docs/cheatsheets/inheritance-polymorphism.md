...
| `reinterpret_cast` | No | Low-level bit punning |
...
* `classes-constructors-raii.rst` — destructor mechanics
* `composition-vs-inheritance.rst` — when NOT to inherit
* `oop-principles-solid.rst` — LSP and Open/Closed in depth
* `advanced-oop-patterns.rst` — Visitor pattern, CRTP
* `raii-smart-pointers.rst` — polymorphic ownership with unique_ptr
