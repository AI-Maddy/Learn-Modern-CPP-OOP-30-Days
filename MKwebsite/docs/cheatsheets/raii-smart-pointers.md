...
* `classes-constructors-raii.rst` — custom RAII classes
* `rule-of-5-cheat.rst` — copy/move semantics of owning types
* `move-semantics-gotchas.rst` — moving unique_ptr correctly
* `inheritance-polymorphism.rst` — polymorphic ownership with unique_ptr
* `advanced-oop-patterns.rst` — factory methods returning unique_ptr
