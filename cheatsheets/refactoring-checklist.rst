...
Two code paths that look 80% the same.  Extract the common logic and
parameterise the differences.
...
Interfaces or base classes that exist "in case they're needed later" but
have only one implementor.
...
+-----+-------------------------------------+-------------------------------------+
| 2   | Add a test for the target code if   | New test passes                     |
|     | missing                             |                                     |
+-----+-------------------------------------+-------------------------------------+
| 3   | Rename for clarity (variable,       | Tests still green; names express     |
|     | function, class)                    | intent                               |
+-----+-------------------------------------+-------------------------------------+
| 4   | Extract method: move a block into   | Tests still green; function named   |
|     | a named function                    | after what it does                   |
+-----+-------------------------------------+-------------------------------------+
| 5   | Extract class: move related         | Tests still green; each class has   |
|     | functions + data into a new type    | single responsibility                |
+-----+-------------------------------------+-------------------------------------+
...
When a derived class uses only part of a base class, or the IS-A relationship
is forced, prefer composition:
...
Incrementally replace a legacy system by routing new traffic to the
refactored implementation while keeping the old one running:
