...
+============================+=============================+====================+
| ``FILE* f = fopen(...)``   | ``ScopedFile f{...}``       | (no stdlib equiv.) |
| ``fclose(f)`` (forget?)    | auto-closed in destructor   |                    |
+----------------------------+-----------------------------+--------------------+
| ``new T``                  | custom RAII wrapper         | ``std::unique_ptr``|
| ``delete p`` (forget?)     |                             |                    |
+----------------------------+-----------------------------+--------------------+
| ``mtx.lock()``             | ``ScopedLock lk{mtx}``      | ``std::lock_guard``|
| ``mtx.unlock()`` (forget?) | auto-unlocked               |                    |
+----------------------------+-----------------------------+--------------------+
