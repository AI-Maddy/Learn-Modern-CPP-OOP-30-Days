...
Allows an empty base subobject to share the address of another member,
removing the 1-byte overhead of empty member storage:
...
Every class with a virtual function gets a hidden ``vptr`` (pointer to the
vtable), added as the first member of the object.
...
Virtual inheritance avoids the "diamond" duplication problem but adds
an extra layer of indirection (vbptr or offset in vtable).
...
Objects that cross cache line boundaries cause extra loads.
...
After placement new over an existing object, the old pointer becomes invalid.
``std::launder`` tells the compiler "this pointer points to a new live object
at this address":
