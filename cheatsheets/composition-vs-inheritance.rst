...
Inject behavior as a composited object (or interface). Swappable at
construction time or runtime.
...
Mixins add reusable behavior to a class without a full inheritance hierarchy.
Implemented in C++ via CRTP (see ``crtp-static-polymorphism.rst``) or simple
multiple inheritance of interface-only classes.
...
Forward method calls to a contained object. More explicit than inheritance,
keeps the wrapper's interface narrow.
...
``class D : private B`` means "D is implemented-in-terms-of B". It is
composition with controlled exposure of base members. Rarely needed; prefer
