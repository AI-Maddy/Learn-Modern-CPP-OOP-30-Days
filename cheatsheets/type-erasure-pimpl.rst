...
The destructor **must** be defined in the .cpp where ``Impl`` is complete.
If you ``= default`` it in the header, ``unique_ptr<Impl>``'s destructor
tries to call ``delete`` on an incomplete type — hard compile error.
...
+------------------------+----------------------------------+
| Destructor in .cpp     | Destructor can be defaulted in   |
| mandatory              | header (shared_ptr stores deleter)|
+------------------------+----------------------------------+
...
Type erasure stores objects of **any** type that satisfies some interface,
without requiring a common base class and without exposing the concrete type.
...
+===================+============+==================+=====================================+
| ``std::function`` | Open       | Exactly one      | Callbacks, event handlers,          |
|                   |            | call signature   | strategy pattern                    |
+-------------------+------------+------------------+-------------------------------------+
| ``std::any``      | Open       | None (manual     | Configuration bags, generic         |
|                   |            | any_cast needed) | property maps, scripting bridges    |
+-------------------+------------+------------------+-------------------------------------+
| ``std::variant``  | Closed     | Visitor/get      | Discriminated unions, AST nodes,    |
|                   |            |                  | error-or-value, state machines      |
+-------------------+------------+------------------+-------------------------------------+
| Hand-rolled       | Open       | Rich virtual     | Value-semantic polymorphism,        |
| type eraser       |            | interface        | SBO needed, no RTTI environments    |
+-------------------+------------+------------------+-------------------------------------+
| pImpl             | N/A        | ABI boundary     | Library headers, stable ABI,        |
|                   |            | hiding           | slow compile time reduction          |
+-------------------+------------+------------------+-------------------------------------+
...
+=====================+==================+===================+===================+
| ``std::function``   | Yes (unless SBO) | Virtual call      | Deep copy of      |
|                     |                  |                   | callable          |
+---------------------+------------------+-------------------+-------------------+
| ``std::any``        | Yes (unless SBO, | RTTI any_cast     | Deep copy         |
|                     | typically <=16B  |                   |                   |
|                     | inlined)         |                   |                   |
+---------------------+------------------+-------------------+-------------------+
| ``std::variant``    | No (stack union) | Indexed dispatch  | Copies active     |
|                     |                  | (fast)            | member            |
+---------------------+------------------+-------------------+-------------------+
| Hand-rolled eraser  | Configurable     | Virtual call      | Clone virtual fn  |
|                     | (SBO possible)   |                   | needed            |
+---------------------+------------------+-------------------+-------------------+
| pImpl               | Yes (one alloc   | Pointer           | Deep copy if      |
|                     | at construction) | indirection       | implemented       |
+---------------------+------------------+-------------------+-------------------+
