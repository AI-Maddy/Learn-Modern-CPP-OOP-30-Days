...
A function should do one thing and have a name that says what it does.
If you need a comment to explain the body, consider extracting a function.
...
Data members should be ``private``.  Expose only through a documented API.
This allows invariant enforcement and future implementation changes.
...
A reference parameter means the caller retains ownership.
A function returning a reference must ensure the referent outlives the reference.
...
Prefer ``std::sort`` over a hand-rolled sort; ``std::string`` over
``char[]``; ``std::span`` over raw pointer + size.
