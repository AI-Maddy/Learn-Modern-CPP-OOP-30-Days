...
Models a computation that either succeeds with ``T`` or fails with error ``E``.
Header: ``<expected>``
...
+---------------------+-------------------+------------------+--------------------+
| Caller must handle  | No (silent skip   | Yes (warning if  | No (can ignore)    |
|                     | possible)         | not checked)     |                    |
+---------------------+-------------------+------------------+--------------------+
| Performance         | Zero cost on      | Zero overhead    | Zero overhead      |
|                     | happy path;       | always           | always             |
|                     | expensive on throw|                  |                    |
+---------------------+-------------------+------------------+--------------------+
| Composability       | try/catch blocks  | .and_then chain  | Manual if/return   |
+---------------------+-------------------+------------------+--------------------+
| Binary/embedded     | Often disabled    | Always available | Always available   |
| environments        |                   |                  |                    |
+---------------------+-------------------+------------------+--------------------+
| Stack unwinding     | Yes               | No               | No                 |
+---------------------+-------------------+------------------+--------------------+
| Use for             | Truly exceptional | Expected failure | C API integration, |
|                     | conditions, third | paths, parsing,  | POSIX errno-style  |
|                     | party library     | validation,      | error reporting    |
|                     | integration       | IO results       |                    |
+---------------------+-------------------+------------------+--------------------+
